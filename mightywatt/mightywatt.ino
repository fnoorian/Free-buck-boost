...
void serialMonitor()
{
  if (Serial.available() > 0)  
  {
    ...
    // Wait for receiving start of JSON
    while (Serial.read() != '{');
    ...
    int end = Serial.readBytesUntil('}',cmd,BUFF_MAX);
    ...
    // Handle set commands
    if(cmd[1] == '='){
      sscanf(cmd, "%[^= ] = %d",cmd,&val);  // Parse set commands
      if(!stricmp(cmd,"I")){
    ...
    //Serial.flush();         // Flush the serial receive buffer
